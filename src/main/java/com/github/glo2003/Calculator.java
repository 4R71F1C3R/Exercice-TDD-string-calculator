...
public class Calculator {
    ...
    public int add(String numbers) {
        ...
        String bodySection = delimiterSectionAndBody.get(1);

        String delimiterRegex = parseDelimiters(delimiterSection);

        List<Integer> parsedNumbers = parseNumbers(bodySection, delimiterRegex);
    ...
    private String parseDelimiters(String delimiterSection) {
        ...
        if (delimiterSection.startsWith(DELIMITER_SECTION_START)) {
            ...
            if (delimiterText.charAt(0) == MULTICHAR_DELIMITER_START) {
                ...
                } while (delimiterText.startsWith(String.valueOf(MULTICHAR_DELIMITER_START)));
            } else {
    ...
    private Stream<Integer> filterNumbers(List<Integer> numbers) {
        return numbers.stream().filter(x -> x<= MAX_NUMBER);
    }
}
